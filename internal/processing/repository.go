...
func (r *Repository) FinishRetry(ctx context.Context, jobID, token, summary string, delay time.Duration) error {
	...
	_, err = tx.Exec(ctx, `UPDATE jobs SET status=$3, lease_token=NULL, leased_until=NULL, next_attempt_at=CASE WHEN $3='ready' THEN now()+$4::interval ELSE next_attempt_at END,last_error=$5,updated_at=now() WHERE id=$1 AND lease_token=$2`, jobID, token, status, delay.String(), summary)
	if err != nil {
		return err
	}
...
func (r *Repository) finishFailure(ctx context.Context, jobID, token, summary string, delay time.Duration, permanent bool) error {
	...
	if _, err = tx.Exec(ctx, `UPDATE jobs SET status='dead_letter',lease_token=NULL,leased_until=NULL,last_error=$3,updated_at=now() WHERE id=$1 AND lease_token=$2`, jobID, token, summary); err != nil {
		return err
	}
...
func (r *Repository) FinishExtraction(ctx context.Context, jobID, token string, snapshot ExtractionSnapshot) error {
	...
	if _, err = tx.Exec(ctx, `UPDATE jobs SET status='succeeded',lease_token=NULL,leased_until=NULL,last_error=NULL,updated_at=now() WHERE id=$1 AND lease_token=$2`, jobID, token); err != nil {
		return err
	}
...
func (r *Repository) RecoverExpiredLeases(ctx context.Context) (int64, error) {
	...
	defer func() { _ = tx.Rollback(context.Background()) }()
	rows, err := tx.Query(ctx, `SELECT id,document_id,lease_token,attempts,max_attempts FROM jobs WHERE status='running' AND leased_until<=now() FOR UPDATE SKIP LOCKED`)
	if err != nil {
		return 0, err
	}
	defer rows.Close()
	var jobs []struct {
		id, documentID, token string
		attempts, max         int
	}
	for rows.Next() {
		var j struct {
		...
		if err := rows.Scan(&j.id, &j.documentID, &j.token, &j.attempts, &j.max); err != nil {
			return 0, err
		}
		jobs = append(jobs, j)
	}
	...
	for _, j := range jobs {
		...
		if _, err := tx.Exec(ctx, `UPDATE jobs SET status=$2,lease_token=NULL,leased_until=NULL,next_attempt_at=CASE WHEN $2='ready' THEN now() ELSE next_attempt_at END,updated_at=now(),last_error='lease expired' WHERE id=$1`, j.id, status); err != nil {
			return 0, err
		}
