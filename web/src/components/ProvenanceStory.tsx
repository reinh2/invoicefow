...
function Field({ label, value, tone, note }: { label: string; value: string; tone: FieldTone; note?: string }): ReactElement {
  return <div className={`story-field story-field-${tone}`}>
...
const steps: Step[] = [
  {
    ...
    panel: <div className="story-paper" aria-hidden="true">
  ...
  },
  {
    ...
    panel: <div className="story-fields">
      <Field label="Supplier" value={demo.supplier} tone="extracted" />
      <Field label="Invoice number" value={demo.number} tone="extracted" />
      <Field label="Line 1" value={demo.lineDescription} tone="extracted" note={`${demo.quantity} × ${demo.unitPrice} ${demo.currency}`} />
      <Field label="Total" value={`${demo.total} ${demo.currency}`} tone="extracted" note="Exact minor units, never a browser float" />
    </div>,
  },
  {
    state: 'Corrected',
    ...
    panel: <div className="story-fields">
      <Field label="Line 1 · version 1" value={demo.lineDescription} tone="extracted" note="Extracted proposal, retained" />
      <Field label="Line 1 · version 2" value={demo.correctedDescription} tone="edited" note="Human correction" />
      <Field label="Total" value={`${demo.total} ${demo.currency}`} tone="extracted" note="Unchanged by this edit" />
    </div>,
  },
  {
    state: 'Approved',
    ...
    panel: <div className="story-fields">
      <Field label="Approved version" value="Version 2" tone="approved" note="Referenced immutably on the document" />
      <Field label="CSV export" value={`invoice-…-v2.csv · ${demo.total} ${demo.currency}`} tone="approved" note="Repeatable, byte-identical" />
      <Field label="Webhook export" value="HMAC-SHA256 signed, server-owned destination" tone="approved" note="Bounded retries, then dead-letter" />
    </div>,
  },
];
