...
it('rejects a malformed webhook export response as invalid_response', async () => {
  globalThis.fetch = vi.fn().mockResolvedValue(
    new Response(JSON.stringify({ export: { id: 42, status: 'pending' } }), {
      status: 202,
      headers: { 'Content-Type': 'application/json' },
    }),
  );

  await expect(triggerWebhookExport('0d0c2342-2486-4f10-a858-e75bc763f3e4')).rejects.toMatchObject({
...
it('accepts a complete safe export projection', async () => {
  const exportRecord = {
    id: 'exp-12345678',
    document_id: '0d0c2342-2486-4f10-a858-e75bc763f3e4',
    version_number: 1,
    export_type: 'webhook',
    status: 'pending',
  ...
  globalThis.fetch = vi.fn().mockResolvedValue(
    new Response(JSON.stringify({ export: exportRecord }), {
      status: 202,
      headers: { 'Content-Type': 'application/json' },
    }),
  );
